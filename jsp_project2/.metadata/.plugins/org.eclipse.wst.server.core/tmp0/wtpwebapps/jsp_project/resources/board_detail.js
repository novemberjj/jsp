...
async function postCommentToServer(cmtData){
	try{
		const url = "/cmt/post"; 
		//url : 데이터 보낼 곳의 경로
		const config={ //config : 보낼 내용의 정보
			method:'post', //method : 전송 방식
			headers:{ //headers : 전송 데이터의 타입, 인코딩
				'Content-Type':'application/json; charset=utf-8'
			},
			body: JSON.stringify(cmtData) //전송내용 JSON데이터를 스트링으로 변환
		};
		const response = await fetch(url, config); //해당 url, config로 데이터 보냄
		const result = await response.text(); //결과로 텍스트 파일 받음
		// isOk - controller에서 보내는 결과
